...
export async function GET() {
  try {
    ...
    const [connections, checkpoints, smokeTests, calls] = await Promise.all([
      ...
      db.from("maintenance_tickets").select("id", { count: "exact", head: true }).eq("provider", "elevenlabs"),
    ]);
    for (const result of [connections, checkpoints, smokeTests, calls]) if (result.error) throw new ApiError(result.error.code === "42P01" ? "missing_migration" : "server_error", "Unable to load integration health.");
    ...
    const definitions = [
      { id: "elevenlabs", configured: configured(["ELEVENLABS_API_KEY", "ELEVENLABS_AGENT_ID", "ELEVENLABS_WEBHOOK_SECRET"]), checkpoint: checkpoint("elevenlabs"), importedCount: calls.count || 0, webhookConfigured: Boolean(process.env.ELEVENLABS_WEBHOOK_SECRET?.trim()) },
    ...
    const providers = definitions.map(item => {
      ...
      const status = item.connection?.status || (!item.configured ? "missing_configuration" : authenticated ? "authenticated" : item.checkpoint?.last_success_at ? "healthy" : item.id === "elevenlabs" ? "configured" : "needs_authorization");
      return {
        provider: item.id,
        ...
        importedCount: "importedCount" in item ? item.importedCount : null,
        importedLastRun: Number((item.checkpoint?.checkpoint as Record<string, unknown> | null)?.importedInLastRun || 0),
        webhookConfigured: "webhookConfigured" in item ? item.webhookConfigured : null,
      };
    });
