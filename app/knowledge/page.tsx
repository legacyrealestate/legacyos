...
const categories = ["Policies", "Maintenance", "Residents", "Owners", "Leasing", "Vendors", "Operations"];
    ...
    <section className="mx-auto max-w-6xl">
      ...
      <div className="mt-7 grid gap-8 lg:grid-cols-[330px_minmax(0,1fr)]">
        <form onSubmit={upload} className="border border-black/[.08] bg-white p-6">
          <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center bg-black text-white"><Upload size={18} /></span><div><h2 className="font-semibold">Add knowledge</h2><p className="text-xs text-zinc-500">PDF, DOCX, TXT, Markdown, CSV, JSON</p></div></div>
          ...
          <label className="mt-5 block text-xs font-medium text-zinc-700">File<input name="file" type="file" accept=".pdf,.docx,.txt,.md,.markdown,.csv,.json" className="mt-2 block w-full border border-dashed border-black/[.18] p-3 text-xs" /></label>
          ...
          <p className="mt-4 text-xs leading-5 text-zinc-500">Image OCR is intentionally unavailable until an OCR provider is configured. No uploaded file is public.</p>
        </form>
