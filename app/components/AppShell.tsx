...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="legacy-shell min-h-screen bg-[#090b0a] text-zinc-100 md:flex">
    <header className="fixed inset-x-0 top-0 z-[200] flex h-[68px] items-center justify-between border-b border-white/10 bg-[#0d100f]/95 px-5 backdrop-blur md:hidden"><span className="text-lg font-semibold tracking-tight text-white">LegacyOS</span><div className="flex items-center gap-4"><button aria-label="Open notifications" onClick={() => setNotificationsOpen(true)} className="relative"><Bell size={19}/>{unacknowledgedCount > 0 && <span className="absolute -right-2 -top-2 rounded-full bg-red-600 px-1.5 text-[10px] text-white">{unacknowledgedCount}</span>}</button><button aria-label="Open navigation" onClick={() => setMobileOpen(!mobileOpen)}>{mobileOpen ? <X/> : <Menu/>}</button></div></header>
    <aside className={`fixed inset-y-0 left-0 z-[300] w-[290px] border-r border-white/10 bg-[#0d100f] transition-transform duration-200 md:hidden ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}><div className="flex h-full flex-col p-5 pt-24"><Navigation pathname={pathname} onNavigate={() => setMobileOpen(false)}/><div className="mt-auto"><AccountControls compact onNavigate={() => setMobileOpen(false)}/></div></div></aside>
    ...
    <aside className="sticky top-0 hidden h-screen w-[272px] shrink-0 flex-col border-r border-white/10 bg-[#0d100f] p-6 md:flex"><div><p className="text-[10px] font-semibold uppercase tracking-[.25em] text-zinc-500">Legacy Real Estate</p><h1 className="mt-3 text-[30px] font-semibold tracking-tight text-white">LegacyOS</h1><p className="mt-3 max-w-[205px] text-sm leading-5 text-zinc-400">Leasing inbox, call intelligence, and operational memory.</p></div><div className="mt-10 min-h-0 flex-1 overflow-y-auto"><Navigation pathname={pathname}/></div><button onClick={() => setNotificationsOpen(true)} className="mt-5 flex h-11 items-center justify-between rounded-xl border border-white/10 px-3 text-sm text-zinc-200 hover:bg-white/5"><span className="flex items-center gap-3"><Bell size={17}/>Notifications</span>{unacknowledgedCount > 0 && <span className="rounded-full bg-red-600 px-2 py-0.5 text-[10px] text-white">{unacknowledgedCount}</span>}</button>{notificationError && <p className="mt-2 text-xs text-red-400">{notificationError}</p>}<div className="mt-4"><AccountControls/></div></aside>
    <main className="min-w-0 flex-1 overflow-y-auto bg-[#090b0a] p-4 pt-[86px] md:p-8">{account && account.profileStatus !== "active" ? <div className="mx-auto mt-16 max-w-xl rounded-2xl border border-amber-200 bg-white p-8"><p className="text-[10px] uppercase tracking-[.24em] text-amber-700">Account access</p><h1 className="mt-4 text-3xl font-semibold">Profile {account.profileStatus}</h1><p className="mt-4 text-sm leading-6 text-zinc-600">Signed in as <strong>{account.email}</strong>. {account.profileStatus === "pending" ? "This account is awaiting administrator approval." : "This profile is inactive. Contact an administrator."}</p><button onClick={() => endLegacySession()} className="mt-6 rounded-xl bg-black px-5 py-3 text-sm text-white">Sign out and use another account</button></div> : children}</main>
