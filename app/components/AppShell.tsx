...
function Navigation({ pathname, onNavigate }: { pathname: string; onNavigate?: () => void }) {
  ...
  const item = (link: Item, compact = false) => {
    ...
    return <Link key={link.href} href={link.href} onClick={onNavigate} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm transition ${active ? "border border-emerald-100 bg-[#edf8f1] text-[#12643e] shadow-[0_6px_16px_rgba(18,100,62,.08)]" : "text-zinc-600 hover:bg-zinc-100 hover:text-zinc-950"}`}>
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#eef1ee] text-zinc-950 md:flex">
    ...
    <aside className="sticky top-0 hidden h-screen w-[272px] shrink-0 flex-col border-r border-black/[.07] bg-[#fbfcfb] p-6 md:flex"><div><p className="text-[10px] font-semibold uppercase tracking-[.25em] text-zinc-400">Legacy Real Estate</p><div className="mt-3 flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-emerald-500"/><h1 className="text-[30px] font-semibold tracking-tight">LegacyOS</h1></div><p className="mt-3 max-w-[205px] text-sm leading-5 text-zinc-500">Leasing inbox, call intelligence, and operational memory.</p></div><div className="mt-10 min-h-0 flex-1 overflow-y-auto"><Navigation pathname={pathname}/></div><button onClick={() => setNotificationsOpen(true)} className="mt-5 flex h-11 items-center justify-between rounded-xl border border-black/[.08] bg-white px-3 text-sm shadow-sm hover:bg-zinc-50"><span className="flex items-center gap-3"><Bell size={17}/>Notifications</span>{unacknowledgedCount > 0 && <span className="rounded-full bg-red-600 px-2 py-0.5 text-[10px] text-white">{unacknowledgedCount}</span>}</button>{notificationError && <p className="mt-2 text-xs text-red-700">{notificationError}</p>}<div className="mt-4"><AccountControls/></div></aside>
    <main className="min-w-0 flex-1 overflow-y-auto bg-[#eef1ee] p-4 pt-[86px] md:p-8"><div className="mx-auto max-w-[1680px]">{account && account.profileStatus !== "active" ? <div className="mx-auto mt-16 max-w-xl rounded-2xl border border-amber-200 bg-white p-8"><p className="text-[10px] uppercase tracking-[.24em] text-amber-700">Account access</p><h1 className="mt-4 text-3xl font-semibold">Profile {account.profileStatus}</h1><p className="mt-4 text-sm leading-6 text-zinc-600">Signed in as <strong>{account.email}</strong>. {account.profileStatus === "pending" ? "This account is awaiting administrator approval." : "This profile is inactive. Contact an administrator."}</p><button onClick={() => endLegacySession()} className="mt-6 rounded-xl bg-black px-5 py-3 text-sm text-white">Sign out and use another account</button></div> : children}</div></main>
