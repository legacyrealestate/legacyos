...
export default function AlmaDock() {
  ...
  async function send() {
    ...
    try {
      ...
      setMessages((current) => [...current, { role: "assistant", content: response.ok ? data.message : data.error || "I couldn’t complete that request.", citations: response.ok ? data.citations || [] : [] }]);
    } catch {
  ...
  return (
    <>
      {open && <div className="fixed bottom-24 right-4 z-[450] flex h-[min(620px,72vh)] w-[calc(100vw-2rem)] max-w-[420px] flex-col overflow-hidden rounded-[26px] border border-white/10 bg-[#0b0e0c] text-white shadow-2xl md:right-7">
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-4">{messages.map((message, index) => <div key={index} className={`max-w-[88%] rounded-2xl p-3 text-sm leading-6 ${message.role === "user" ? "ml-auto bg-emerald-400 text-black" : "bg-white/8 text-zinc-200"}`}><p>{message.content}</p>{message.citations?.length ? <p className="mt-2 border-t border-white/10 pt-2 text-[10px] leading-4 text-zinc-400">Sources: {message.citations.map((citation) => citation.label).join(" · ")}</p> : null}</div>)}{loading && <div className="w-fit rounded-2xl bg-white/8 px-4 py-3 text-xs text-zinc-400">Reading live operations…</div>}</div>
