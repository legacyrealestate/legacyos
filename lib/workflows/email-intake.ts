...
export async function processEmailIntake(messageId:string){
  ...
  if(actionable){
    let draft=leasing ? leasing.missing.length ? leasingInformationRequest({name:contact.full_name,missing:leasing.missing}) : leasingAcknowledgement({name:contact.full_name,property:thread.property}) : leasingAcknowledgement({name:contact.full_name,property:thread.property});
    const recentMessages=(await db.from("email_messages").select("id").eq("thread_id",thread.id).gte("created_at",new Date(Date.now()-3600000).toISOString())).data||[];
    const recentIds=recentMessages.map(item=>item.id);
    const recentOutbound=recentIds.length?(await db.from("email_outbound_actions").select("id").in("source_message_id",recentIds).eq("status","sent")).data||[]:[];
    const automatic = Boolean(leasing && canAutoSend({classification:classification.primary,confidence:classification.confidence,requiresHuman:classification.requiresHuman,automationDisabled:thread.automation_disabled,headers,sender:message.sender,ownAddresses:[connection.account_email||""],threadOutboundLastHour:recentOutbound.length}));
    if(!automatic&&process.env.AI_ASSISTANCE_ENABLED==="true")try{draft=await generateEmailDraft({subject:String(message.subject||""),body,contactName:contact.full_name,property:thread.property});}catch{}
    outbound=await requestEmailAction(connection.user_id,messageId,{action:automatic?"reply":"draft",to:[sender.email],body:draft,idempotencyKey:`email-${automatic?"auto-reply":"draft"}-${messageId}`},{allowAutomatedSend:automatic});
  }
  const automatedSent=outbound?.status==="sent";
  const decision=classification.requiresHuman?"human_review_required":automatedSent?"approved_reply_sent":actionable?"draft_created":"no_reply_needed";
